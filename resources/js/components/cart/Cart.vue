<template>
    <div class="drag">
        <div className="cart-container">
            <h2>Shopping Cart</h2>
            <div v-if="$store.state.Pizzasstore.cart.length < 0">
                ...
                <div class="cart-summary">
                    ...
                    <div class="cart-checkout">
                        <div class="subtotal">
                            ...
                            <span class="amount"
                                >{{
                                    $store.state.Pizzasstore.cartTotal
                                }}
                                TND</span
                            >
                        </div>
...
const clearCart = () => {
    store.commit("Pizzasstore/clearCart");
};
    ...
    Produits.value.map((pro) => {
        if (pro.id == item.product.id) {
            if (item.qty < pro.qtestock) {
                store.commit("Pizzasstore/plusCart", item);
            } else alert("Quantité stock indisponible");
        }
    });
};
const minus = (item) => {
    ...
    } else store.commit("Pizzasstore/minusCart", item);
};
const getProduits = () => {
    axios
        .get("/api/pizzas/")
